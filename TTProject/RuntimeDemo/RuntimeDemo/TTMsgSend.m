...
+ (void)testMsgSend {
    ...
    TTMsgSend *msg = ((TTMsgSend * (*)(id, SEL))objc_msgSend)((id)[TTMsgSend class], @selector(alloc));

    msg = ((TTMsgSend *(*)(id, SEL))objc_msgSend)((id)msg, @selector(init));
...
//    int d =  ((int (*)(id, SEL, NSString *, int))objc_msgSend)((id)msg, @selector(hasArguments:andReturnValue:), @"", 2012);
    ...
    //add method
    class_addMethod([TTMsgSend class], NSSelectorFromString(@"sayHello2"), (IMP)sayHello, "v@:");
    TTMsgSend *instance = [[TTMsgSend alloc] init];
